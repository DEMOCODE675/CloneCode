...
class Plugin(ManifestDrivenPlugin):
    """Detect Vue projects from package metadata."""
    ...
    def detect(self, snapshot: ProjectSnapshot) -> PluginAssessment:
        """Detect Vue projects with dependency evidence."""

        assessment = super().detect(snapshot)
        if dependency_present(snapshot, "vue", "@vue/runtime-core"):
            assessment = replace(assessment, score=assessment.score + 5.0, reasons=assessment.reasons + ("vue dependency found",))
