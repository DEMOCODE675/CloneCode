...
class Plugin(ManifestDrivenPlugin):
    """Detect Astro projects from package metadata."""
    ...
    def detect(self, snapshot: ProjectSnapshot) -> PluginAssessment:
        """Detect Astro projects with dependency evidence."""

        assessment = super().detect(snapshot)
        if dependency_present(snapshot, "astro"):
            assessment = replace(assessment, score=assessment.score + 5.0, reasons=assessment.reasons + ("astro dependency found",))
